...
Original file is located at
    https://colab.research.google.com/drive/1X56NpQ5S7da-xTAIRq3SvC_kBdZ7NOl4
"""
...
fc_net.py 파일 수정 이후 아래 과정을 진행해 테스트하시면 됩니다.
"""
...
Batch Norm과 weight initialization의 상호 작용을 확인해보는 실험입니다.
"""
...
## Answer:
작은 가중치 초기화에서 batch norm이 없는 network는 아무것도 학습하지 못하는 반면, batch norm을 사용하는 network는 학습에 성공합니다. 이는 효과적인 backpropagation을 수행할 수 있을만큼 데이터를 scale up 하기 때문입니다.

반면에 weight scale이 너무 크면, batch norm이 적용된 network조차도 normalization 과정이 특정 feature를 너무 크게 왜곡하기 때문에 효율성이 떨어지게 됩니다.
...
이번에는 batch norm과 batch size의 상호작용을 연구하기 위한 테스트입니다.
"""
...
## Answer:
훈련시 : batch size를 키우면 sampling이 실제 모집단에 더 가까워지므로 더 정확해질 수 있습니다. 즉, batch size가 클수록 batch norm은 더 좋은 결과를 얻을 수 있습니다. batch size가 작으면 훈련 정확도가 낮아집니다.

테스트시 : batch size는 영향을 주지 않습니다.

(When the batch size is small, the training accuracy of bn will be lower, because during training, bn uses the statistics in a batch, so there is a problem that the statistics are not accurate. When the batch is small, Especially obvious, when the batch is larger, the effect will be better, but in the test mode, the problem is not obvious, because the test mode uses the running statistic, which is similar to the global statistic of the data, so it is tested. In test mode, batch size has no effect on bn.)
...
## Answer:
1과 2는 layer norm과 비슷하고, 3은 batch norm과 비슷합니다.
...
## Answer:
2 뉴런의 차원이 너무 작으면 통계 차이가 너무 커서 모델의 성능이 매우 많이 변동될 수 있습니다.
