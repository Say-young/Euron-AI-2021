...
FOLDERNAME = FOLDERNAME = 'cs231n/assignments/assignment1/cs231n/'
...
Hyperparameter은 epoch를 계속 돌면서 최적의 해를 찾아나간다. 처음에는 작은 수로 시작하고, 작은 수에서 찾은 최적의 범위를 기반으로 더욱 정교한 수를 찾아낸다.
이때 우리는 train set에만 적용되는 overfitting parameter을 찾지 않도록 주의하여야 한다.
"""
...
- 더 큰 데이터셋에서 학습하는 것은 더 좋은 성과를 불러온다.
- hidden unit을 늘리는 것은 대체적으로 좋은 결과를 가져올 수 있으나, 일정 수준 이상의 hidden unit은 오히려 안 좋은 효과를 가져올 수 있다. (모델이 deep하다고 무조건 좋은 것은 아니다.)
- regularization은 모델에 fit한 정도에 맞춰 설정하여야 하며, 무조건 increase한다고 좋은 결과가 나타나지는 않는다.
---
# IMPORTANT
